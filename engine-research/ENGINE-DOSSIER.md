...
### 9co. ⭐⭐⭐ THE ONE-FRAME FLICKER: for one tick the exact map is built from the OTHER EYE'S projection — caught by hand four times; the eye lock keeps one eye (2026-09-22 evening, LIVE VR by Tefa + build on RTX; INSTALLED, NOT WORN)

**How it was caught.** The change measure had failed as a detector (18 spikes vs ~20 flickers one run, 0 vs ~20 the next) and `hold 2` had not removed them, so the plugin got a hand trigger: the last 90 shown scope pictures kept on the GPU and written to disk with one line of numbers per frame when the wearer pulls the trigger (`flicker_ring.cpp`, note 2026-09-22c). Tefa, standing still, shot on each flicker. Four rings, and in each exactly one frame differs from both neighbours while the neighbours agree with each other — and **on exactly that frame the per-frame line carries `P20 = −0.174`, the other eye's off-centre projection term, against `+0.174` on every other frame** `[verified-live 2026-09-22, n=4 of 4]`. Map centre and source pointer are unchanged on it. The odd frame is the same scene at a different framing (brightened crops in note 2026-09-22e), i.e. the 09-13 stills.

**Mechanism.** `world_tick.cpp` reads `get_ProjectionMatrix` off the camera every tick into `g_cam_P`; the exact map (`sg_compute_P`, `P real`) builds the crop from it. In VR the camera's projection is swapped per eye, and the game-thread read lands on one eye's nearly always and on the other's now and then (thread timing — "random"). That tick's crop is placed for the wrong eye. Everything built for the flicker since 09-16 (`hold`, spike dump, map hold) sat downstream of that read. The 09-13 "shake, per-eye projection alternating, `eye moved`" was the same fault seen the other way. The measure's `d` on a flicker frame is 0.026 — under the 0.08 threshold and under the 4× rule in a moving scene — which is why no knob ever caught one `[verified-live 2026-09-22]`.

**The fix: `eye_lock.h`.** Learn the majority eye over the first 30 off-centre reads, then reject reads whose `P[8]` sign is the other eye's; the map keeps the last accepted projection; symmetric (flat) projections are never rejected. Rejections are counted and logged once a second — the count should match the flickers the wearer used to see, so a run tells "no effect" from "wrong idea" on its own. `re_scope_eyelock.txt` 0/1/2 (off / on / relearn), `EYE-LOCK-OFF/ON.bat`. `tools/eye_lock_test.cpp` 6/6 `[verified-numerically 2026-09-22]`; plugin `8c188a22…` `[compile-verified 2026-09-22]`, installed, archived.
